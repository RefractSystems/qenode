...
# Add project root to path for vproto and other tools
sys.path.append(str(Path(__file__).parent.parent.parent)) # noqa: TID251
...
def list_interfaces() -> None:
    print("interface {value=virtmcu0}{display=VirtMCU Zenoh Capture}") # noqa: T201

def list_dlts() -> None:
    print("dlt {number=147}{name=DLT_USER0}{display=VirtMCU Custom Link Type}") # noqa: T201

def list_config() -> None:
    print("arg {number=0}{call=--session}{display=Zenoh Session}{type=string}{default=tcp/localhost:7447}{tooltip=Zenoh router endpoint}") # noqa: T201
    print("arg {number=1}{call=--topic}{display=Zenoh Topic}{type=string}{default=sim/coord/**/rx}{tooltip=Zenoh topic to subscribe to}") # noqa: T201
    print("arg {number=2}{call=--legacy}{display=Use Legacy Topics}{type=boolflag}{default=false}{tooltip=Subscribe to sim/comm/** for raw traffic}") # noqa: T201
        ...
        try:
            ...
            try:
                ...
                if "sim/coord/" in str(topic):
                    if len(data) >= 29:
                        src = int.from_bytes(data[0:4], "little")
                        dst = int.from_bytes(data[4:8], "little")
                        vtime = int.from_bytes(data[8:16], "little")
                        # seq is data[16:24]
                        proto = data[24]
                        plen = int.from_bytes(data[25:29], "little")
                        
                        payload = data[29:29+plen]
                        dumper.write_packet(vtime, src, dst, proto, payload)
                        return
            ...
            try:
                if len(data) >= 24:
                    header = vproto.ZenohFrameHeader.unpack(data[:24])
                    payload = data[24:24+header.size]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--session", default="tcp/localhost:7447")
    ...
    if args.extcap_interfaces:
        ...
        if not args.fifo:
            print("Error: --fifo is required for capture", file=sys.stderr) # noqa: T201
            sys.exit(1)
