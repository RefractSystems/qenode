    ...
    match val {
        ...
        serde_yaml::Value::String(s) => {
            ...
            if let Some(stripped) = s.strip_prefix("0x") {
                ...
                s.parse::<u64>()
                    .expect("Invalid decimal string for address")
            }
        }
                ...
                serde_yaml::Value::String(s) => {
                    ...
                    if k_qemu == "macaddr" {
                        ...
                        dts.push_str(&format!(
                            ...
                            phandles
                                .get(s)
                                .expect("phandle should exist as it was just checked")
                        ));
                    } else {
                        dts.push_str(&format!("{}    {} = \"{}\";\n", indent, k_qemu, s));
                    }
        ...
        if let Some(props) = &mut p.properties {
            if let Some(serde_yaml::Value::String(svd_path)) = props.get("svd") {
                ...
                if svd_path_buf.exists() {
                    ...
                    props.insert(
                        "svd-hash".to_string(),
                        serde_yaml::Value::Number(svd_hash.into()),
                    );
...
"#,
        ...
        assert!(platform
            .dts_content
            .contains("compatible = \"s32k144-lpuart\""));
