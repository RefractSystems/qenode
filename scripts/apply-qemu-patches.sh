...
QEMU_DIR_RAW="${1:-}"
if [ -z "$QEMU_DIR_RAW" ] || [ ! -d "$QEMU_DIR_RAW" ]; then
    echo "Usage: $0 <path_to_qemu_src>"
    exit 1
fi

# Ensure QEMU_DIR is an absolute path
QEMU_DIR="$(cd "$QEMU_DIR_RAW" && pwd)"
...
if [ -f hw/riscv/virt.c ]; then
    ...
    if ! grep -q "virtmcu-transport-hub" hw/riscv/virt.c; then
        sed -i 's/machine_class_allow_dynamic_sysbus_dev(mc, "mmio-socket-bridge");/machine_class_allow_dynamic_sysbus_dev(mc, "mmio-socket-bridge");\n    machine_class_allow_dynamic_sysbus_dev(mc, "virtmcu-transport-hub");/' hw/riscv/virt.c
    fi
    # virtmcu: add devices to sysbus-fdt bindings so they bypass the FDT generation check
    if ! grep -q "virtmcu-transport-hub" hw/core/sysbus-fdt.c; then
        sed -i 's/TYPE_BINDING("mmio-socket-bridge", no_fdt_node),/TYPE_BINDING("mmio-socket-bridge", no_fdt_node),\n    TYPE_BINDING("virtmcu-transport-hub", no_fdt_node),/' hw/core/sysbus-fdt.c
    fi
...
# 3. Inject 'subdir('virtmcu')' into QEMU's hw/meson.build
if ! grep -q "subdir('virtmcu')" hw/meson.build; then
    echo "  -> Injecting subdir('virtmcu') into hw/meson.build..."
    echo "subdir('virtmcu')" >> hw/meson.build
fi

# 4. Update Meson version to support objects in Rust targets
...
# Finalize: commit all modifications to ensure the submodule is clean
if git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    # Add specifically known untracked files generated by patches
    [ -d include/virtmcu ] && git add include/virtmcu/
    [ -f net/virtmcu.c ] && git add net/virtmcu.c
    if [ -n "$(git status --porcelain -uno)" ]; then
        echo "  -> Committing automated modifications to QEMU..."
        git commit -a -m "virtmcu: applied automated hooks and sed patches"
    fi
fi
