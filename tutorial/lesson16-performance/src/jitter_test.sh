#!/usr/bin/env bash
# tutorial/lesson16-performance/src/jitter_test.sh — Jitter Injection Determinism Test
#
...
cleanup() {
    ...
    bash "$WORKSPACE_DIR/scripts/cleanup-sim.sh" --quiet 2>/dev/null || true
}
trap cleanup EXIT
...
ROUTER_URL="tcp/127.0.0.1:${ROUTER_PORT}"
...
PROXY_URL="tcp/127.0.0.1:${PROXY_PORT}"
echo "--- Starting jitter proxy on ${PROXY_URL} (upstream: ${ROUTER_URL}) ---"
python3 "$SCRIPT_DIR/jitter_proxy.py" "$ROUTER_URL" "$PROXY_PORT" "$MAX_JITTER_US" &
PROXY_PID=$!
sleep 2
...
dtb = os.path.join(os.path.dirname(SCRIPT_DIR), "minimal.dtb")
kernel = os.path.join(os.path.dirname(SCRIPT_DIR), "bench.elf")
...
run_sh = os.path.join(WORKSPACE, "scripts", "run.sh")
