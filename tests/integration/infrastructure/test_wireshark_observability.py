...
async def test_wireshark_extcap_capture(zenoh_router: str, zenoh_session: zenoh.Session, tmp_path: Path) -> None:
    ...
    extcap_cmd = [
        "python3", "tools/wireshark/virtmcu_extcap.py",
        "--capture",
        "--fifo", str(pcap_path),
        "--session", zenoh_router,
        "--topic", "sim/coord/**/rx"
    ]
    ...
    async with ManagedSubprocess("extcap", extcap_cmd) as _proc:
        ...
        # Create a CoordMessage payload (matching main.rs encode_message)
        # src(4), dst(4), vtime(8), seq(8), proto(1), len(4), payload
        vtime = 1_234_567_890
        src = 1
        dst = 2
    ...
    packet_start = 24
    ts_sec = int.from_bytes(data[packet_start:packet_start+4], "little")
    ts_usec = int.from_bytes(data[packet_start+4:packet_start+8], "little")
    ...
    p_src = int.from_bytes(data[dlt_start:dlt_start+4], "little")
    p_dst = int.from_bytes(data[dlt_start+4:dlt_start+8], "little")
    p_proto = int.from_bytes(data[dlt_start+8:dlt_start+10], "little")
