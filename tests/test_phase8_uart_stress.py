...
_STRESS_SCRIPT = os.path.join(
    os.path.dirname(__file__), "..", "test", "phase8", "uart_stress_test.sh"
)
...
def test_uart_stress_integration():
    ...
    assert result.returncode == 0, (
        "uart_stress_test.sh exited with non-zero status — "
        "check QEMU logs in /tmp/uart_stress_*/qemu.log"
    )
