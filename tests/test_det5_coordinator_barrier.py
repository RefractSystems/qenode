...
async def test_det5_coordinator_barrier(zenoh_router, zenoh_session):
        ...
        try:
            ...
            for run in range(1, 101):
                ...
                def _send_shuffled(q: int):
                    ...
                    q_payload = struct.pack("<Q", q)
                    for nid in nodes:
                        zenoh_session.put(f"sim/coord/{nid}/done", q_payload)

                await asyncio.to_thread(_send_shuffled, run)
                await asyncio.sleep(0.1) # Wait for processing  # SLEEP_EXCEPTION: deliberate yielding
    ...
    print("STDERR", proc.stderr_text)
    print("STDOUT", proc.stdout_text)
