#!/bin/bash
# ==============================================================================
# Phase 15 Smoke Test: Distribution & Packaging
#
...
TEST_VENV="$REPO_ROOT/.test_venv_phase15"

echo "==> Phase 15: Verifying virtmcu-tools packaging..."
...
cd "$REPO_ROOT"
mkdir -p tests/fixtures/guest_apps/phase15/tmp
source "$TEST_VENV/bin/activate"
yaml2qemu tutorial/lesson15-distribution/src/board.yaml --out-dtb tests/fixtures/guest_apps/phase15/tmp/board.dtb

if [ ! -f tests/fixtures/guest_apps/phase15/tmp/board.dtb ]; then
...
# Cleanup
rm -rf "$TEST_VENV"
rm -rf tests/fixtures/guest_apps/phase15/tmp
