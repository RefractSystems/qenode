...
TMPDIR_LOCAL="$(mktemp -d -t bql_stress_net_XXXXXX)"
QEMU_PID=""
ROUTER_PID=""
...
"$SCRIPTS_DIR/run.sh" --dtb "$TMPDIR_LOCAL/dummy.dtb" -kernel "$TMPDIR_LOCAL/firmware.elf" \
    -netdev virtmcu,id=n0,node=0,router=$ENDPOINT \
    -nographic -monitor none > "$TMPDIR_LOCAL/qemu.log" 2>&1 &
QEMU_PID=$!

sleep "$(( 1 * SLEEP_MULT ))"

python3 "$WORKSPACE_DIR/tests/fixtures/guest_apps/bql_stress/netdev_flood_test.py" "$ENDPOINT"
