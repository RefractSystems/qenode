#!/usr/bin/env bash
# tests/fixtures/guest_apps/complex_board/integration_test.sh
set -euo pipefail
...
# 1. Build firmware
echo "==> Building Radio Test Firmware"
make -C "$SCRIPT_DIR" -q || make -C "$SCRIPT_DIR"

# 2. Start Zenoh Router
echo "==> Starting Zenoh Router"
python3 "$SCRIPT_DIR/zenoh_router.py" "$ROUTER_ENDPOINT" > /dev/null 2>&1 &
sleep 2
...
# Under ASan, everything is significantly slower. Scale the timeout.
MAX_COUNT=20
if [ "${VIRTMCU_USE_ASAN:-0}" = "1" ]; then
    MAX_COUNT=100
fi

echo "Waiting for test to complete (timeout ${MAX_COUNT}s)..."
count=0
while true; do
