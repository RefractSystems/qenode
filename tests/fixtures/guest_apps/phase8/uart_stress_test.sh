...
"$WORKSPACE_DIR/scripts/run.sh" --dtb "$WORKSPACE_DIR/tests/fixtures/guest_apps/phase1/minimal.dtb" \
    -kernel "$WORKSPACE_DIR/tests/fixtures/guest_apps/phase8/echo.elf" \
    -icount shift=6,align=off,sleep=off \
    -device virtmcu-clock,node=0,mode=slaved-icount,router=tcp/127.0.0.1:$PORT, \
...
# Run Stress Test
if python3 "$WORKSPACE_DIR/tests/fixtures/guest_apps/phase8/uart_stress_test.py" "tcp/127.0.0.1:$PORT"; then
