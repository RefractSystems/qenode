#!/usr/bin/env bash
# tests/fixtures/guest_apps/phase7/determinism_test.sh — Phase 7 Zenoh determinism test.
set -euo pipefail
...
"$WORKSPACE_DIR/scripts/run.sh" \
    ...
    -device virtmcu-clock,mode=slaved-icount,node=0,router=tcp/127.0.0.1:$PORT \
...
# Run test
python3 "$WORKSPACE_DIR/tests/fixtures/guest_apps/phase7/test_clock.py" "tcp/127.0.0.1:$PORT"
