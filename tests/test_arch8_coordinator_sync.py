...
@pytest.mark.asyncio
async def test_arch8_coordinator_sync(zenoh_router, zenoh_session, qemu_launcher, tmp_path):
    """
    ARCH-8: TA/Coordinator Synchronization Protocol.
...
"""
    ...
    async def coordinator_loop() -> None:
        """
        Minimal Python coordinator implementing the ARCH-8 barrier protocol.
        ...
        while True:
            ...
            # Simulate coordinator message delivery latency (what ARCH-8 tests).
    ...
    try:
        await wait_for_zenoh_discovery(zenoh_session, "sim/clock/liveliness/1")
        await wait_for_zenoh_discovery(zenoh_session, "sim/clock/liveliness/2")

        # Start emulation.  QMP 'cont' returns as soon as QEMU resumes; the vCPU
        # will block at the first quantum boundary awaiting the VTA GET.
        await n1.start_emulation()
        await n2.start_emulation()
        ...
        assert elapsed >= _COORDINATOR_DELIVERY_DELAY_S, (
            f"ARCH-8 VIOLATION: Clock advanced in {elapsed:.3f}s — "
