...
RADIO_CLI=$(grep "^ieee802154" test/phase14/test.cli || true)
if [ -z "$RADIO_CLI" ] || ! echo "$RADIO_CLI" | grep -q "node=0" || ! echo "$RADIO_CLI" | grep -q "transport=zenoh"; then
    echo "ERROR: Missing or malformed ieee802154 device in CLI: $RADIO_CLI"
    exit 1
fi

HCI_CLI=$(grep "^virtmcu" test/phase14/test.cli || true)
