...
#![allow(dead_code)] // virtmcu-allow: allow reasoning="Pending migration"
#![allow(unused_imports)] // virtmcu-allow: allow reasoning="Pending migration"
#![cfg_attr(
    test,
    allow(
        clippy::expect_used,
...
use virtmcu_qom::{define_prop_macaddr, define_prop_string};
...
extern "C" fn wifi_read(_opaque: *mut c_void, addr: u64, _size: core::ffi::c_uint) -> u64 {
    let s = virtmcu_qom::ffi_call! { &*(_opaque as *mut VirtmcuWifiQEMU) };
    if s.debug {
...
extern "C" fn wifi_write(_opaque: *mut c_void, addr: u64, val: u64, _size: core::ffi::c_uint) {
