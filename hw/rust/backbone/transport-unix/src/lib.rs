...
impl UdsDataTransport {
    ...
    pub fn new_with_fed_id(path: &str, node_id: u32, fed_id: &str) -> Result<Self, String> {
        ...
        let reg_payload = virtmcu_api::encode_uds_registration(node_id, fed_id);
        ...
        ARENA.with(|arena_cell| {
            ...
            Ok(virtmcu_api::TransportReservation::new(topic, buffer, move |vtime, seq| {
                ...
                let fb_payload = virtmcu_api::encode_coord_message(
                    ...
                    vtime,
                    seq,
                    virtmcu_api::Protocol::ReferenceLink, // Note: Currently everything via UDS is hardcoded as ReferenceLink.
                    payload,
                );

                let mut stream = stream_clone.lock().expect("unix transport error");
                write_framed(&mut stream, topic, &fb_payload)
                    .map_err(|e| virtmcu_api::TransportError::Other(e.to_string()))
            }))
        })
    }
    ...
    fn test_reserve_commit_round_trip() {
        ...
        while start.elapsed() < TIMEOUT_2_SEC {
            ...
            if !rx_lock.is_empty() {
                ...
                let (_vtime, _seq, data) = virtmcu_api::decode_coord_message(payload).unwrap();
    ...
    fn test_concurrent_reserve_commit() {
        ...
        t_rx.subscribe(
            ...
            Box::new(move |_topic: &str, payload: &[u8]| {
                ...
                let (vtime, _seq, user_data) = virtmcu_api::decode_coord_message(payload).unwrap();
