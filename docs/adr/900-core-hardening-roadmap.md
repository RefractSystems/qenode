...
*   **Zenoh Session Isolation**: Enforced client-mode isolation for Zenoh sessions in parallel tests, eliminating cross-talk races between virtmcu-test-runner workers.
...
1.  **Static Analysis**: `make test-lint` enforces bans on raw `Mutex<T>`, `thread::sleep`, and `asyncio.sleep` (without annotations).
