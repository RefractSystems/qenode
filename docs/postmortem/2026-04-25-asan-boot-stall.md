...
## Executive Summary
Tests executing under AddressSanitizer (ASan) started failing unpredictably during the `test_phase7` and `test_phase8` milestones with `CLOCK STALL` timeout errors. The timeouts occurred exclusively at `vtime=0`. We found that the extensive overhead imposed by ASan on QEMU's TCG (Tiny Code Generator) block translation during the emulator's first few instructions caused the simulator to exceed the `clock` peripheral's default 5-second `stall-timeout`.
...
The `clock` peripheral enforced the 5-second `stall-timeout` uniformly across all quanta. Since the very first `VirtualTimeAuthority.step(0)` waits for the first quantum boundary, it triggered a stall detection before the boot sequence could complete.
...
```python
extra_args = ["-S", "-device", f"clock,node=0,mode=slaved-icount,router={zenoh_router},stall-timeout=60000"]
```
### Why this was bad:
...
3. **Seamless Inheritance**: Both the Python orchestrator's `VirtualTimeAuthority` (which calculates `_DEFAULT_VTA_STEP_TIMEOUT_S`) and the QEMU `clock` Rust plugin automatically inherit this scaled value.
...
3. [x] Reverted the failed `is_first_quantum` logic inside `clock`.
