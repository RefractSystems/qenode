...
- **Assumption: Network Queues are bounded.** `crossbeam_channel::unbounded()` was used for `netdev` to escape the BQL. If a Zenoh publisher floods the topic faster than the QEMU CPU processes interrupts, the QEMU process will eventually OOM. This was not tested.
