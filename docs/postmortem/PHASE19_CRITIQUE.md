...
- **Restore `ieee802154`**: We must backport the 669-line MAC layer logic from before the Phase 19 migration into the pure-Rust QOM structure.
- **Add runtime BQL locking**: All Zenoh subscriber callbacks *must* wrap QEMU state changes in `virtmcu_bql_lock() / virtmcu_bql_unlock()`. (Applied to `ui` and `chardev`).
- **Improve Smoke Tests**: Phase 14 (`ieee802154`) needs a real runtime execution test, not just a CLI parse check.
- **Stress Tests**: We need a multithreaded test script that hammers `chardev` and `ui` from an external Zenoh node while the guest is running, ensuring no segfaults occur due to missing BQL locks.
