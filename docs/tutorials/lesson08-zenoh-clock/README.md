# Lesson 8: Zenoh Clock: Deterministic Co-simulation Time Synchronization
...
consistent with physical simulation time.  The external `Physical Node` (running in
...
```
Physical Node (Rust, MuJoCo container)
...
zclock_quantum_hook()  (called at every TCG translation-block boundary)
      ...
      ▼
Physical Node receives reply: current_vtime_ns
```
...
1. The `Physical Node` sends a `delta_ns` via Zenoh. `on_query` deposits this delta, wakes the hook, and blocks waiting for the quantum to complete.
...
6. `on_query` wakes, reads the new `vtime_ns`, and sends the Zenoh reply back to the `Physical Node`.
...
QEMU's instruction counter drives virtual time. `on_query` performs the exact same handshake as suspend mode to guarantee strict causal consistency. The `qemu_icount_bias` is advanced in Step 8 of the hook (after the handshake), not directly in `on_query`. The Physical Node waits for the hook to complete the quantum before receiving a reply. Use this when firmware timestamps need sub-quantum precision.
...
time.sleep(1)  # virtmcu-allow: sleep reasoning="waiting for infrastructure  # wait for QEMU to register queryable"
...
vCPU starts ──► hook() [needs_quantum=true]
                   ...
                   ▼
    on_query() ◄───┘ (Zenoh: Physical Node sends delta_ns)
    ...
    on_query() wakes
        │ read vtime_ns
        │ send Zenoh reply (current_vtime_ns) ──► Physical Node
```
...
protocol.  The other half is the `Physical Node` in `tools/cyber_bridge`
