...
The codebase is divided into three primary build domains, each with its own toolchain and lifecycle. For a quick initialization of all domains, run `cargo xtask setup-dev`.

### A. Dependencies (C/C++)
*   **Components**: `zenoh-c` (for native plugin communication) and `flatcc` (for telemetry serialization).
*   **Lifecycle**: Built once during `cargo xtask bootstrap` or pre-compiled into the project's Docker base image.
...
*   **Lifecycle**: 
    *   The **Core** is built initially via `cargo xtask bootstrap` and rarely changes.
    *   **Plugins** are rebuilt frequently via `cargo xtask build` whenever `hw/rust/` code is modified.
...
VirtMCU provides a deterministic way to locate the compiled QEMU binary, supporting complex CI scenarios and custom development setups. This is implemented via `virtmcu-run` (Rust).
...
3.  **No Unannotated Sleeps**: The build includes a linter that rejects any `thread::sleep` or `tokio::time::sleep` not explicitly marked as a `SLEEP_EXCEPTION`, preventing non-determinism from creeping into the codebase.
ng into the codebase.
